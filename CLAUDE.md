...
의류 쇼핑몰 셀러용 리뀑 분석 B2B SaaS. 셀러가 리뷰 CSV/XLSX 를 업로드하면 상품별 반복 이슈,
CS 답글 초안, 상세페이지 보완 힌트를 정리해 준다.

- 프론트엔드: React (Vite), `frontend/`
- 백엔드: Node.js Express + SQLite (better-sqlite3), `backend/`
- 배포: 단일 Render Web Service (Express 가 `frontend/dist` 정적 서빙 + SPA fallback)

## VibeTrack 작업 귀칙

이 프로젝트는 VibeTrack MCP로 상태를 추적한다. 반드시 지켜라.
이 프로젝트의 VibeTrack projectId: `cmreyd4v00001ch4lqsvgpy2n` (모든 vibetrack 도구 호출에 사용)
