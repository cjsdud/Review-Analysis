...
> **“리뷰 엑셀만 올리면, 상품별 반복 불만과 상세페이지 수정안을 빠르게 정리해 드립니다.”**
...
### 베타 데모 샘플 리뷰 데이터 (다운로드용)

직접 업로드 흐름(컬럼 매핑 → 리뷰 분석)까지 체험할 수 있도록 다운로드 가능한 데모 데이터를 제공합니다.

- 다운로드 위치(앱): **리뷰 파일 업로드** 화면 하단 “샘플 리뷰 데이터로 먼저 테스트해보기”
- 파일:
  - `frontend/public/samples/reviewfit_beta_demo_reviews.xlsx`
  - `frontend/public/samples/reviewfit_beta_demo_reviews.csv`
  - (배포 시 `/samples/...` 경로로 서빙)
- 구성: **상품 8개 · 리뷰 360건**. 컬럼: `리뷰ID · 상품명 · 옵션명 · 별점 · 작성일 · 리뷰내용`
- 작성일이 최근 약 90일 범위로 분산되어 **기간별 리뷰 변화** 기능까지 확인 가능
- 상품별로 사이즈·색상·소재·품질·기장·배송 등 반복 이슈가 의도적으로 섞여 있음

> **이 데이터는 기능 시연/베타 테스트용 데모 데이터이며, 실제 고객 리뷰가 아닙니다.**
> 개인정보(이메일·전화번호·주문번호 등)가 포함되어 있지 않습니다.

**개발/QA 기준 인사이트 파일**: `sample-data/reviewfit_beta_demo_expected_insights.json`
— 위 데모 데이터로 분석했을 때 기대되는 상품별 핵심 이슈/감성 흐름을 정리한 회귀 검증 기준 파일입니다.
사용자 UI 에는 노출하지 않으며, 분석 로직 변경 시 결과가 크게 어긋나지 않는지 확인하는 용도입니다.
...
2. **업로드** → CSV/XLSX 리뷰 파일 업로드 (업로드 즉시 개인정보 마스킹 · 플랫폼 구분 없이 컬럼 매핑 단계에서 자동 정렬)
...
3. **파일 업로드**
   - 랜딩 → “리뷰 파일 업로드” → 파일 첨부. (플랫폼 선택 없이 바로 업로드 — 컬럼 차이는 다음 단계에서 자동 매핑)
...
> 현재 업로드 화면에는 플랫폼 선택 UI 가 없으며 모든 업로드는 `source='custom'` (공통 후보만) 으로
> 동작합니다. `SOURCE_FIELD_CANDIDATES` 는 코드에 보존되어 있어, 추후 파일 헤더 기반 자동 추정
> 같은 방식으로 재활용할 수 있습니다. 아래 표는 그 보존된 후보 사전의 설명입니다.

`source` 가 지정되면 `SOURCE_FIELD_CANDIDATES` 의 후보가 함께 적용되고,
...
- **비용 절감**을 위해 LLM에 전체 리뷰를 넣지 않습니다.
  ...
  3. **하이브리드 감성**: 별점만 보지 않고 별점 + 리뷰 문장의 만족/불만 표현을 함께 고려.
     - 별점 4~5 + 약한 이슈만 있으면 **positive 유지** (개선 이슈가 있다고 negative 로 떨어지지 않음)
     - 별점 1~2 + 강한 불만(환불/하자/못 입/단독으로 입기 어려워 등) → **negative 유지**
     - 별점 3 은 neutral 기본 — 명확한 만족/불만 표현이 있을 때만 flip
     - 부정어 반전(`"비침이 거의 없어요"`, `"줄어들지 않았어요"`, `"마감이 나쁘지 않아요"`) → 부정 카운트 X
     - 별점이 문자열(`"5점"`, `"평점 4"`) 로 들어와도 정규화하여 동일 동작
     - 긍정/중립/부정은 **전체 만족도** 판단이고, 개선 이슈는 별도 개념 — 긍정 리뷰 안에서도 별도로 감지될 수 있음 (부정 리뷰 수와 개선 이슈 리뷰 수가 다를 수 있음)
