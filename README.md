...
- **전체 이슈 보기** — 핵심 문제 5개에 표시되지 않은 낮은 우선순위 이슈까지 모두 확인.
  카테고리 필터, 정렬(많이 나온 순/심각도 높은 순/카테고리순), 이슈별 "관련 리뷰 보기" 액션.
- **원본 리뷰 보기** — 이 상품의 분석에 사용된 마스킹된 리뷰 데이터.
  긍정/중립/부정/개선 이슈 있음·없음 필터, 카테고리·별점 필터, 본문 검색, 정렬(최신순/별점/이슈 많은 순),
  각 리뷰 카드의 "업로드 데이터 보기" 펼침으로 상품명·옵션·작성일·답글·리뷰ID 확인 가능.
  현재 필터링된 목록을 CSV 로 내보낼 수 있습니다.

개인정보(전화/이메일/주문번호)는 업로드 직후 마스킹되며, 원본 리뷰 모달도 항상 마스킹된
데이터만 표시합니다.
